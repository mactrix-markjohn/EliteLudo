...
public class SettingsWindowController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (PlayerPrefs.GetInt(StaticStrings.MusicKey, 0) == 1)
        {
            BackgroundMusic.GetComponent<Toggle>().isOn = false;
        }
        ...
        BackgroundMusic.GetComponent<Toggle>().onValueChanged.RemoveAllListeners();
        ...
        BackgroundMusic.GetComponent<Toggle>().onValueChanged.AddListener((value) =>
                {
                    PlayerPrefs.SetInt(StaticStrings.MusicKey, value ? 0 : 1);
                    Debug.Log("MUSIC KEY IS : "+PlayerPrefs.GetInt(StaticStrings.SoundsKey));
                    if ((PlayerPrefs.GetInt(StaticStrings.SoundsKey, 0) == 0) && value == true)
                    {
                        GameManager.Instance.initMenuScript.BackgroundSound.Play();
                    }
                    else
                    {
        ...
        Vibrations.GetComponent<Toggle>().onValueChanged.AddListener((value) =>
                {
                    ...
                    Debug.Log("hello====" +  PlayerPrefs.GetInt(StaticStrings.VibrationsKey ));
